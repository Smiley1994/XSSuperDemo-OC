...
static HFPopViewLogStyle _logStyle;


@interface HFPopViewManager : NSObject

/** 单例 */
HFPopViewManager *popViewManager(void);

@end

@interface HFPopViewManager ()
...
/** 储存待移除的popView  */
@property (nonatomic,strong) NSHashTable <HFPopView *> *removeList;
/** 内存信息View */
@property (nonatomic,strong) UILabel *infoView;

@end

@implementation HFPopViewManager

static HFPopViewManager *_instance;

HFPopViewManager *popViewManager() {
    return [HFPopViewManager sharedInstance];
}
...
+ (NSArray *)getAllPopView {
    return popViewManager().popViewMarr;
}
/** 获取当前页面所有popView */
+ (NSArray *)getAllPopViewForParentView:(UIView *)parentView {
    NSMutableArray *mArr = popViewManager().popViewMarr;
    NSMutableArray *resMarr = [NSMutableArray array];
    for (id obj in mArr) {
        HFPopView *popView = (HFPopView *)obj;
...
+ (NSArray *)getAllPopViewForPopView:(HFPopView *)popView {
    NSArray *mArr = [self getAllPopViewForParentView:popView.parentView];
    NSMutableArray *resMarr = [NSMutableArray array];
    for (id obj in mArr) {
        HFPopView *tPopView = (HFPopView *)obj;
...
+ (void)savePopView:(HFPopView *)popView {
    [popViewManager().popViewMarr addObject:popView];
    
    //优先级排序
    [self sortingArr];
    
    if (_logStyle & HFPopViewLogStyleWindow) {
        [self setInfoData];
    }
    if (_logStyle & HFPopViewLogStyleConsole) {
...
+ (void)removePopView:(HFPopView *)popView {
    if (!popView) { return; }
    NSArray *arr = popViewManager().popViewMarr;
    for (id obj in arr) {
        HFPopView *tPopView = (HFPopView *)obj;
...
+ (void)removeAllPopViewComplete:(HF_Block_Void)complete {
    NSMutableArray *arr = popViewManager().popViewMarr;
    if (arr.count <= 0) {
        complete ? complete() : nil;
        return;
    }
    
    NSArray *popViewMarr = [NSArray arrayWithArray:popViewManager().popViewMarr];
...
+ (void)removeLastPopViewComplete:(HF_Block_Void)complete {
    NSPointerArray *showList =  popViewManager().showList;
    ...
    for (NSInteger i = showList.count - 1; i >= 0; i --) {
        HFPopView *popView = [showList pointerAtIndex:i];
        if (popView) {
...
+ (void)setInfoData {
    popViewManager().infoView.text = [NSString stringWithFormat:@"S:%zd R:%zd",popViewManager().popViewMarr.count,popViewManager().removeList.allObjects.count];
}
...
+ (void)sortingArr {
    ...
    for (int i = 0; i < arr.count; i++) {
        for (int j = i+1; j < arr.count; j++) {
            HFPopView *iPopView = (HFPopView *)arr[i];
            
            HFPopView *jPopView = (HFPopView *)arr[j];
...
/** 转移popView到待移除队列 */
+ (void)transferredToRemoveQueueWithPopView:(HFPopView *)popView {
    NSArray *popViewMarr = popViewManager().popViewMarr;
    
    [popViewMarr enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL * stop) {
        HFPopView *tPopView = (HFPopView *)obj;
        
        if ([popView isEqual:tPopView]) {
            [popViewManager().removeList addObject:obj];
            [popViewManager().popViewMarr removeObject:obj];
        }
    }];
}

+ (void)destroyInRemoveQueue:(HFPopView *)popView {
    if (popViewManager().removeList.allObjects.count <= 0) {
...
- (NSHashTable<HFPopView *> *)removeList {
...
@interface HFPopView () <UIGestureRecognizerDelegate>
...
/** 代理池 */
@property (nonatomic, strong) NSHashTable <id<HFPopViewProtocol>> *delegates;
/** 背景点击手势 */
...
+ (nullable instancetype)initWithCustomView:(UIView *_Nonnull)customView {
    return [self initWithCustomView:customView popStyle:HFPopStyleNO dismissStyle:LSTDismissStyleNO];
}
...
+ (nullable instancetype)initWithCustomView:(UIView *_Nonnull)customView
    ...
    HFPopView *popView = [[HFPopView alloc] initWithFrame:popViewFrame];
...
- (void)dealloc {
    ...
    //从待移除队列中销毁
    [HFPopViewManager destroyInRemoveQueue:self];
    [self lst_PopViewDidDismissForPopView:self];
    
    for (HF_Block_Void block in _popViewDidDismissBlocks.copy) {
        block ? block() : nil;
    }
...
- (void)removeFromSuperview {
    ...
    [HFPopViewManager transferredToRemoveQueueWithPopView:self];
}
...
- (void)lst_PopViewBgClickForPopView:(HFPopView *)popView {
    for (id<HFPopViewProtocol> delegate in _delegates.copy) {
...
- (void)setDelegate:(id<HFPopViewProtocol>)delegate {
...
- (void)popWithPopStyle:(HFPopStyle)popStyle duration:(NSTimeInterval)duration isOutStack:(BOOL)isOutStack {
    ...
    if (self.isSingle) {//单显
        NSArray *popViewArr = [HFPopViewManager getAllPopViewForPopView:self];
        for (id obj  in popViewArr) {//移除所有popView和移除定a时器
            HFPopView *lastPopView = (HFPopView *)obj;
            ...
            if (popViewArr.count >= 1) {
                ...
                if (self.isStack) {//堆叠显示
                    ...
                    if (self.priority >= lastPopView.priority) {//置顶显示
                        ...
                        [HFPopViewManager savePopView:self];
                        return;
                    }
                }
            } else {
...
//    }
    if (!self.superview) {
        ...
        [popViewManager().showList addPointer:(__bridge void * _Nullable)self];
    }
    ...
    //保存popView
    if (!isOutStack) { [HFPopViewManager savePopView:self]; }
}

- (void)popAnimationWithPopStyle:(HFPopStyle)popStyle duration:(NSTimeInterval)duration {
    
    LSTPopViewWK(self);
    if (popStyle == HFPopStyleFade) {//渐变出现
        self.backgroundView.backgroundColor = [self getNewColorWith:self.bgColor alpha:0.0];
    ...
    } else if (popStyle == HFPopStyleNO){//无动画
        self.backgroundView.backgroundColor = [self getNewColorWith:self.bgColor alpha:0.0];
...
- (void)hanlePopAnimationWithDuration:(NSTimeInterval)duration
    ...
    LSTPopViewWK(self);
    switch (popStyle) {
        case HFPopStyleScale:// < 缩放动画，先放大，后恢复至原大小
        {
            ...
            break;
        case HFPopStyleSmoothFromTop:
        case HFPopStyleSmoothFromBottom:
        case HFPopStyleSmoothFromLeft:
        case HFPopStyleSmoothFromRight:
        case HFPopStyleSpringFromTop:
        case HFPopStyleSpringFromLeft:
        case HFPopStyleSpringFromBottom:
        case HFPopStyleSpringFromRight:
        {
            CGPoint startPosition = self.customView.layer.position;
            if (popStyle == HFPopStyleSmoothFromTop || popStyle == HFPopStyleSpringFromTop) {
                self.customView.layer.position = CGPointMake(startPosition.x, -_customView.pv_Height*0.5);
                
            } else if (popStyle == HFPopStyleSmoothFromLeft || popStyle == HFPopStyleSpringFromLeft) {
                self.customView.layer.position = CGPointMake(-_customView.pv_Width*0.5, startPosition.y);
                
            } else if (popStyle == HFPopStyleSmoothFromBottom || popStyle == HFPopStyleSpringFromBottom) {
            ...
            CGFloat damping = 1.0;
            if (popStyle == HFPopStyleSpringFromTop||
                popStyle == HFPopStyleSpringFromLeft||
                popStyle == HFPopStyleSpringFromBottom||
                popStyle == HFPopStyleSpringFromRight) {
                damping = 0.65;
            }
            ...
            break;
        case HFPopStyleCardDropFromLeft:
        case HFPopStyleCardDropFromRight:
        {
            CGPoint startPosition = self.customView.layer.position;
            if (popStyle == HFPopStyleCardDropFromLeft) {
            ...
            [UIView animateWithDuration:duration*0.6 animations:^{
                wk_self.customView.layer.transform = CATransform3DMakeRotation(LST_DEGREES_TO_RADIANS((popStyle == HFPopStyleCardDropFromRight) ? 5.5 : -5.5), 0, 0, 0);
            } completion:^(BOOL finished) {
                [UIView animateWithDuration:duration*0.2 animations:^{
                    wk_self.customView.transform = CGAffineTransformMakeRotation(LST_DEGREES_TO_RADIANS((popStyle == HFPopStyleCardDropFromRight) ? -1.0 : 1.0));
                } completion:^(BOOL finished) {
                    [UIView animateWithDuration:duration*0.2 animations:^{
...
- (void)dismissWithDismissStyle:(LSTDismissStyle)dismissStyle
    ...
    if (!self.isSingle && (isRemove && [HFPopViewManager getAllPopViewForPopView:self].count >= 1)){//多显
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(resDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            //popView出栈
            if (!self.isStack && [HFPopViewManager getAllPopViewForPopView:self].count >= 1) {
                NSArray *popViewArr = [HFPopViewManager getAllPopViewForPopView:self];
                id obj = popViewArr[popViewArr.count-1];
                HFPopView *tPopView = (HFPopView *)obj;
                if (!tPopView.superview) {
                    [tPopView.container addSubview:tPopView];
                    [popViewManager().showList addPointer:(__bridge void * _Nullable)tPopView];
                }
                [tPopView popWithPopStyle:HFPopStyleFade duration:0.25 isOutStack:YES];
            }
        });
    }
    if (isRemove) {
...
- (NSTimeInterval)getPopDuration:(NSTimeInterval)currDuration {
    ...
    if (_popStyle == HFPopStyleNO) { return 0.0f; }
    
    if (_popStyle == HFPopStyleFade) { return 0.2f; }
    ...
    if (self.popDuration == LSTPopViewDefaultDuration) {
        ...
        if (_popStyle == HFPopStyleScale) {
            defaultDuration = 0.3f;
        }
        if (_popStyle == HFPopStyleSmoothFromTop ||
            _popStyle == HFPopStyleSmoothFromLeft ||
            _popStyle == HFPopStyleSmoothFromBottom ||
            _popStyle == HFPopStyleSmoothFromRight ||
            _popStyle == HFPopStyleSpringFromTop ||
            _popStyle == HFPopStyleSpringFromLeft ||
            _popStyle == HFPopStyleSpringFromBottom ||
            _popStyle == HFPopStyleSpringFromRight ||
            _popStyle == HFPopStyleCardDropFromLeft ||
            _popStyle == HFPopStyleCardDropFromRight) {
...
//            NSLog(@"isX=%@,isY=%@,velocityX=%lf,velocityY=%lf",isX?@"YES":@"NO",isY?@"YES":@"NO",velocityX,velocityY);
        }else {//普通拖拽
...
- (NSHashTable<id<HFPopViewProtocol>> *)delegates {
...
/** 保存popView */
+ (void)savePopView:(HFPopView *)popView {
    [HFPopViewManager savePopView:popView];
}

/** 获取全局(整个app内)所有popView */
+ (NSArray *)getAllPopView {
    return [HFPopViewManager getAllPopView];
}

/** 获取当前页面所有popView */
+ (NSArray *)getAllPopViewForParentView:(UIView *)parentView {
    return [HFPopViewManager getAllPopViewForParentView:parentView];
}

/** 获取当前页面指定编队的所有popView */
+ (NSArray *)getAllPopViewForPopView:(HFPopView *)popView {
    return [HFPopViewManager getAllPopViewForPopView:popView];
}
 ...
 */
+ (HFPopView *)getPopViewForKey:(NSString *)key {
    return [HFPopViewManager getPopViewForKey:key];
}

/** 移除popView */
+ (void)removePopView:(HFPopView *)popView {
    [HFPopViewManager removePopView:popView];
}
...
+ (void)removePopViewForKey:(NSString *)key {
    [HFPopViewManager removePopViewForKey:key complete:nil];
}

+ (void)removePopViewForKey:(NSString *)key complete:(HF_Block_Void)complete
{
    [HFPopViewManager removePopViewForKey:key complete:complete];
}

/** 移除所有popView */
+ (void)removeAllPopView {
    [HFPopViewManager removeAllPopViewComplete:nil];
}

+ (void)removeAllPopViewComplete:(HF_Block_Void)complete
{
    [HFPopViewManager removeAllPopViewComplete:complete];
}

/** 移除 最后一个弹出的 popView */
+ (void)removeLastPopView {
    [HFPopViewManager removeLastPopViewComplete:nil];
}

+ (void)removeLastPopViewComplete:(HF_Block_Void)complete
{
    [HFPopViewManager removeLastPopViewComplete:complete];
}
...
+ (void)setLogStyle:(HFPopViewLogStyle)logStyle {
    ...
    if (logStyle == HFPopViewLogStyleNO) {
        [popViewManager().infoView removeFromSuperview];
        popViewManager().infoView = nil;
    }
}

...
// 获取当前设备类型 e.g. XSDeviceTypeIPhone  XSDeviceTypeIPad
+ (void)setDeviceType:(XSDeviceType)deviceType{}
+ (XSDeviceType)deviceType{
...
// 当前设备的 CPU 类型
+ (void)setDeviceCPUType:(XSCPUType)deviceCPUType{}
+ (XSCPUType)deviceCPUType{
    ...
    switch (hostInfo.cpu_type) {
        ...
        case CPU_TYPE_X86_64:
            return XSCPUTypeX86_64;
            break;
            
        default:
            return XSCPUTypeUnkown;
            break;
    }
}
...
+ (NSString *)deviceNetType{
    ...
    XSReachability *reach = [XSReachability reachabilityForInternetConnection];
    XSNetworkStatus status = [reach currentReachabilityStatus];
    switch (status) {
        case XSNotReachable:
            networktype = @"无服务";
            break;
        case XSReachableViaWiFi:
            networktype = @"Wi-Fi";
            break;
        case XSReachableViaWWAN:
            networktype = @"WWAN";
            break;
        default:
            break;
    }
    if (status != XSReachableViaWWAN) {
        return networktype;
    }
    // 如果是移动网络，判断具体

...
@implementation UIDevice (XSDeviceInfo)
...
+ (BOOL)getDeviceIsIPhone{
    return [UIDevice getDeviceType] == XSDeviceTypeIPhone;
}
...
+ (BOOL)getDeviceIsIPad{
    return [UIDevice getDeviceType] == XSDeviceTypeIPad;
}
...
+ (BOOL)getDeviceIsIPod{
    return [UIDevice getDeviceType] == XSDeviceTypeIPod;
}
...
/**
 * 获取当前设备类型
 * e.g. XSDeviceTypeIPhone
 @return 当前设备类型的枚举
 */
+ (XSDeviceType)getDeviceType{
    ...
    if (isSimulator) {
        if (isPhone) {
            return XSDeviceTypeIPhoneSimulator;
        }else if (isPad){
            return XSDeviceTypeIPadSimulator;
        }else{
            return XSDeviceTypeUnkown;
        }
    }
    // 如果是真机
    if (isPhone) {
        return XSDeviceTypeIPhone;
    }else if (isPad){
        return XSDeviceTypeIPad;
    }else if (isPod){
        return XSDeviceTypeIPod;
    }else{
        return XSDeviceTypeIPadSimulator;
    }
}

...
@implementation XSVideoCapturerParam
...
@interface XSVideoCapture () <AVCaptureVideoDataOutputSampleBufferDelegate>
...
@implementation XSVideoCapture

- (instancetype)initWithCaptureParam:(XSVideoCapturerParam *)param error:(NSError *__autoreleasing  _Nullable *)error {
    ...
    if (self = [super init]) {
        ...
        if (captureDeviceArray.count == 0) {
            errorMessage = [self errorWithDomain:@"MAVideoCapture::Get Camera Faild!"];
            return nil;
        }
        ...
        if (errorMessage)
        {
            errorMessage = [self errorWithDomain:@"MAVideoCapture::AVCaptureDeviceInput init error"];
            return nil;
        }
        ...
        if ([self.captureSession canAddInput:self.captureDeviceInput]) {
            ...
            [self errorWithDomain:@"MAVideoCapture::Add captureDeviceInput failed!"];
        ...
        if ([self.captureSession canAddOutput:self.captureVideoDataOutput]) {
            ...
            [self errorWithDomain:@"MAVideoCapture::Add captureVideoDataOutput Faild!"];
        ...
        if ([self.captureSession canAddOutput:self.captureStillImageOutput]) {
            ...
            [self errorWithDomain:@"MAVideoCapture::Add captureStillImageOutput Faild!"];
...
- (NSError *)startCapture {
    ...
    if (self.isCapturing) {
        ...
        return [self errorWithDomain:@"MAVideoCapture::startCapture failed! is capturing!"];
    ...
    if (videoAuthStatus != AVAuthorizationStatusAuthorized) {
        ...
        [self errorWithDomain:@"MAVideoCapture::Camera Authorizate failed!"];
...
- (NSError *)stopCapture {
    ...
    if (!self.isCapturing) {
        ...
        return [self errorWithDomain:@"MAVideoCapture::stopCapture failed! is not capturing!"];
...
- (NSError *)reverseCamera {
    ...
    if (captureDeviceArray.count == 0) {
        return [self errorWithDomain:@"MAVideoCapture::reverseCamera failed! get new Camera Faild!"];
    }
...
//    NSLog(@"frameWidth %lf, frameHeight %lf", frameWidth, frameHeight);
    if ([self.delegate respondsToSelector:@selector(videoCaptureOutputSampleBuffer:)]) {
...
- (NSError *)adjustFrameRate:(NSInteger)frameRate {
    ...
    if (frameRate > frameRateRange.maxFrameRate || frameRate < frameRateRange.minFrameRate) {
        ...
        return [self errorWithDomain:@"MAVideoCapture::Set frame rate failed! out of range"];

...
@implementation XSAudioCapturerParam
...
@property (nonatomic, strong, readwrite) XSAudioCapturerParam *capturerParam;
@property (nonatomic, strong) dispatch_queue_t captureQueue;
@property (nonatomic, assign) BOOL isError;

@end

@implementation XSAudioCapture
...
- (void)setupAudioCaptureInstance:(NSError **)error {
    ...
    asbd.mChannelsPerFrame = (UInt32) self.capturerParam.channels; // 每帧的声道数
    asbd.mFramesPerPacket = 1; // 每个数据包帧数
    asbd.mBitsPerChannel = (UInt32) self.capturerParam.bitDepth; // 采样位深
    ...
    asbd.mSampleRate = self.capturerParam.sampleRate; // 采样率
...
static OSStatus audioBufferCallBack(void *inRefCon,
    ...
    @autoreleasepool {
        XSAudioCapture *capture = (__bridge XSAudioCapture *) inRefCon;
        if (!capture) {
            return -1;
        }

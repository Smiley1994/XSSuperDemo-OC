...
#pragma mark - ===== XSAudioEncoderParam

@implementation XSAudioEncoderParam

- (instancetype)init {
    if (self = [super init]) {
        
        self.audioBitrate = XSAudioBitRate_Default;
...
#pragma mark - ===== XSAudioEncoder

@interface XSAudioEncoder () {
...
@implementation XSAudioEncoder

- (instancetype)initWithAudioEncoderParam:(XSAudioEncoderParam *)param {
    
    if (self = [super init]) {
        
        _audioEncodeParam = param;
        
        _encoderQueue = dispatch_queue_create("com.pusher.audioEncoder", DISPATCH_QUEUE_SERIAL);
...
- (void)encodeSampleBuffer:(CMSampleBufferRef)buffer timeStamp:(uint64_t)timeStamp{
    ...
    dispatch_async(_encoderQueue, ^{
        [weakSelf encodeSampleBufferInternal:buffer timeStamp:timeStamp];
        CFRelease(buffer);
    });
}

- (void)encodeSampleBufferInternal:(CMSampleBufferRef)buffer timeStamp:(uint64_t)timeStamp{
    // 1、从输入数据中获取音频格式信息。
    ...
    if (_leftLength + audioLength >= _bufferLength) {
        ...
        for (NSInteger index = 0; index < encodeCount; index++) {
            [self encodeBuffer:p timing:timingInfo timeStamp:timeStamp]; // 调用编码方法。
            p += _bufferLength;
        }
...
- (void)encodeBuffer:(char *)buffer timing:(CMSampleTimingInfo)timing timeStamp:(uint64_t)timeStamp{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(audioEncoderSampleBufferRef:timeStamp:)]) {
        
        [self.delegate audioEncoderSampleBufferRef:sampleBuffer timeStamp:timeStamp];
...
- (void)setupAudioEncoderInstanceWithInputAudioFormat:(AudioStreamBasicDescription)inputFormat error:(NSError **)error {
    ...
    // 3、设置编码器参数：音频编码码率。
    UInt32 outputBitrate = (UInt32) self.audioEncodeParam.audioBitrate;

...
@implementation XSVideoEncoderParam

- (instancetype)init {
    if ([super init]) {
        self.profileLevel = XSVideoEncoderProfileLevelBP;
...
- (instancetype)initWithParam:(XSVideoEncoderParam *)param {
    ...
    if (self = [super init]) {
        ...
        switch (self.videoEncodeParam.profileLevel) {
            case XSVideoEncoderProfileLevelBP:
                profileRef = kVTProfileLevel_H264_Baseline_3_1;
                break;
            case XSVideoEncoderProfileLevelMP:
                profileRef = kVTProfileLevel_H264_Main_3_1;
                break;
            case XSVideoEncoderProfileLevelHP:
...
void encodeOutputDataCallback(void * CM_NULLABLE outputCallbackRefCon, void * CM_NULLABLE sourceFrameRefCon, OSStatus status, VTEncodeInfoFlags infoFlags, CM_NULLABLE CMSampleBufferRef sampleBuffer) {
    ...
    XSVideoEncoder *encoder = (__bridge XSVideoEncoder *)outputCallbackRefCon;
    ...
    if (isKeyFrame) {
        ...
        if (noErr == spsStatus && noErr == ppsStatus) {
            ...
            if ([encoder.delegate respondsToSelector:@selector(videoEncodeOutputData:isKeyFrame:)]) {
                [encoder.delegate videoEncodeOutputData:spsData isKeyFrame:isKeyFrame];
            }
            ...
            if ([encoder.delegate respondsToSelector:@selector(videoEncodeOutputData:isKeyFrame:)]) {
                [encoder.delegate videoEncodeOutputData:ppsData isKeyFrame:isKeyFrame];
            }
        }
    }
    ...
    while (bufferOffset < totalLength - avcHeaderLength) {
        ...
        if ([encoder.delegate respondsToSelector:@selector(videoEncodeOutputData:isKeyFrame:)])
        {
            [encoder.delegate videoEncodeOutputData:outputFrameData isKeyFrame:isKeyFrame];
        }
    }

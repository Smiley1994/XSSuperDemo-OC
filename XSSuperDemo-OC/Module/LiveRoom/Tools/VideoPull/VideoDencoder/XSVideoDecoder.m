...
-(BOOL)initH264Decoder {
    ...
    callBackRecord.decompressionOutputCallback = videoH264DecodeOutputData;
...
// H264 解码回调函数
static void videoH264DecodeOutputData(void *decompressionOutputRefCon, void *sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef pixelBuffer, CMTime presentationTimeStamp, CMTime presentationDuration) {
    // retain再输出，外层去release
    CVPixelBufferRetain(pixelBuffer);
    XSVideoDecoder *decoder = (__bridge XSVideoDecoder *)decompressionOutputRefCon;
    
    if ([decoder.delegate respondsToSelector:@selector(videoH264DecodeOutputData:)]) {
        [decoder.delegate videoH264DecodeOutputData:pixelBuffer];
    }
}

// H265 解码回调函数
static void videoH265DecodeOutputData(void *decompressionOutputRefCon, void *sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef pixelBuffer, CMTime presentationTimeStamp, CMTime presentationDuration) {
    // retain再输出，外层去release
    CVPixelBufferRetain(pixelBuffer);
    XSVideoDecoder *decoder = (__bridge XSVideoDecoder *)decompressionOutputRefCon;
    
    if ([decoder.delegate respondsToSelector:@selector(videoH265DecodeOutputData:)]) {
        [decoder.delegate videoH265DecodeOutputData:pixelBuffer];
    }
}

...
@interface HFCategoryIndicatorLineView ()
@property (nonatomic, strong) HFCategoryViewAnimator *animator;
@end

@implementation HFCategoryIndicatorLineView
...
- (void)configureDefaulteValue {
    _lineStyle = HFCategoryIndicatorLineStyle_Normal;
...
- (void)hf_contentScrollViewDidScroll:(HFCategoryIndicatorParamsModel *)model {
    ...
    if (self.lineStyle == HFCategoryIndicatorLineStyle_Normal) {
    ...
    }else if (self.lineStyle == HFCategoryIndicatorLineStyle_Lengthen) {
        CGFloat maxWidth = rightX - leftX + rightWidth;
        //前50%，只增加width；后50%，移动x并减小width
        if (percent <= 0.5) {
            targetX = leftX;
            targetWidth = [HFCategoryFactory interpolationFrom:leftWidth to:maxWidth percent:percent*2];
        }else {
            targetX = [HFCategoryFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
            targetWidth = [HFCategoryFactory interpolationFrom:maxWidth to:rightWidth percent:(percent - 0.5)*2];
        }
    }else if (self.lineStyle == HFCategoryIndicatorLineStyle_LengthenOffset) {
        ...
        if (percent <= 0.5) {
            targetX = [HFCategoryFactory interpolationFrom:leftX to:leftX + offsetX percent:percent*2];;
            targetWidth = [HFCategoryFactory interpolationFrom:leftWidth to:maxWidth percent:percent*2];
        }else {
            targetX = [HFCategoryFactory interpolationFrom:(leftX + offsetX) to:rightX percent:(percent - 0.5)*2];
            targetWidth = [HFCategoryFactory interpolationFrom:maxWidth to:rightWidth percent:(percent - 0.5)*2];
        }
    }

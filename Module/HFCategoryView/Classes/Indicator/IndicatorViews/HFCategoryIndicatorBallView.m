//
//  HFCategoryIndicatorBallView.m
//  HFCategoryView
//
//  Created by Macrolor on 2018/8/21.
...
- (void)hf_contentScrollViewDidScroll:(HFCategoryIndicatorParamsModel *)model {
    ...
    if (percent == 0) {
        ...
        if (percent <= 0.5) {
            targetXOfBigBall = [HFCategoryFactory interpolationFrom:leftX to:(rightX - self.ballScrollOffsetX) percent:percent*2];
            targetWidthOfSmallBall = [HFCategoryFactory interpolationFrom:ballWidth to:ballWidth/2 percent:percent*2];
        }else {
            targetXOfBigBall = [HFCategoryFactory interpolationFrom:(rightX - self.ballScrollOffsetX) to:rightX percent:(percent - 0.5)*2];
            targetWidthOfSmallBall = [HFCategoryFactory interpolationFrom:ballWidth/2 to:0 percent:(percent - 0.5)*2];
            targetXOfSmallBall = [HFCategoryFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
        }
    }

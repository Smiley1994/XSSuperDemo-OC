...
@interface HFCategoryListContainerViewController : UIViewController
...
@implementation HFCategoryListContainerViewController
- (void)dealloc
{
...
@interface HFCategoryListContainerView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, weak) id<HFCategoryListContainerViewDelegate> delegate;
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, id<HFCategoryListContentViewDelegate>> *validListDict;
@property (nonatomic, assign) NSInteger willAppearIndex;
@property (nonatomic, assign) NSInteger willDisappearIndex;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) HFCategoryListContainerViewController *containerVC;
@end

@implementation HFCategoryListContainerView

- (instancetype)initWithType:(HFCategoryListContainerType)type delegate:(id<HFCategoryListContainerViewDelegate>)delegate{
...
- (void)initializeViews {
    ...
    _containerVC = [[HFCategoryListContainerViewController alloc] init];
    ...
    if (self.containerType == HFCategoryListContainerType_ScrollView) {
        if (self.delegate &&
...
- (void)layoutSubviews {
    ...
    if (self.containerType == HFCategoryListContainerType_ScrollView) {
        if (CGRectEqualToRect(self.scrollView.frame, CGRectZero) ||  !CGSizeEqualToSize(self.scrollView.bounds.size, self.bounds.size)) {
            ...
            [_validListDict enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull index, id<HFCategoryListContentViewDelegate>  _Nonnull list, BOOL * _Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIView* listView = nil;
    id<HFCategoryListContentViewDelegate> list = _validListDict[@(indexPath.item)];
    if (list != nil) {
...
#pragma mark - HFCategoryViewListContainer
...
- (void)reloadData {
    for (id<HFCategoryListContentViewDelegate> list in _validListDict.allValues) {
...
- (void)initListIfNeededAtIndex:(NSInteger)index {
    ...
    id<HFCategoryListContentViewDelegate> list = _validListDict[@(index)];
    if (list != nil) {
        //列表已经创建好了
        return;
    }

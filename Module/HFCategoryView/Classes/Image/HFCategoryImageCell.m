...
@interface HFCategoryImageCell()
@property (nonatomic, strong) NSString *currentImageName;
@property (nonatomic, strong) NSURL *currentImageURL;
@end

@implementation HFCategoryImageCell
...
- (void)layoutSubviews {
    ...
    HFCategoryImageCellModel *myCellModel = (HFCategoryImageCellModel *)self.cellModel;
...
- (void)reloadData:(HFCategoryBaseCellModel *)cellModel {
    ...
    //因为`- (void)reloadData:(HFCategoryBaseCellModel *)cellModel`方法会回调多次，尤其是左右滚动的时候会调用无数次，如果每次都触发图片加载，会非常消耗性能。所以只会在图片发生了变化的时候，才进行图片加载。

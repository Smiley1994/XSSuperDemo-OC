...
@class HFCategoryBaseView;

@protocol HFCategoryViewListContainer <NSObject>
...
@protocol HFCategoryViewDelegate <NSObject>
...
/**
 ...
 @return 能否点击
 */
- (BOOL)categoryView:(HFCategoryBaseView *)categoryView canClickItemAtIndex:(NSInteger)index;
 ...
 */
- (void)categoryView:(HFCategoryBaseView *)categoryView scrollingFromLeftIndex:(NSInteger)leftIndex toRightIndex:(NSInteger)rightIndex ratio:(CGFloat)ratio;

@end

@interface HFCategoryBaseView : UIView

@property (nonatomic, strong, readonly) HFCategoryCollectionView *collectionView;

@property (nonatomic, strong) NSArray <HFCategoryBaseCellModel *> *dataSource;

@property (nonatomic, weak) id<HFCategoryViewDelegate> delegate;

/**
 高封装度的列表容器，使用该类可以让列表拥有完成的生命周期、自动同步defaultSelectedIndex、自动调用reloadData。
 */
@property (nonatomic, weak) id<HFCategoryViewListContainer> listContainer;
...
/**
 刷新指定的index的cell
 内部会触发`- (void)refreshCellModel:(HFCategoryBaseCellModel *)cellModel index:(NSInteger)index`方法进行cellModel刷新
...
@interface HFCategoryBaseView (UISubclassingBaseHooks)
 ...
 */
- (void)refreshSelectedCellModel:(HFCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(HFCategoryBaseCellModel *)unselectedCellModel NS_REQUIRES_SUPER;
...
/**
 ...
 @param index 选中的index
 @param selectedType HFCategoryCellSelectedType
 @return 返回值为NO，表示触发内部某些判断（点击了同一个cell），子类无需后续操作。
 */
- (BOOL)selectCellAtIndex:(NSInteger)index selectedType:(HFCategoryCellSelectedType)selectedType NS_REQUIRES_SUPER;
 ...
 */
- (void)refreshCellModel:(HFCategoryBaseCellModel *)cellModel index:(NSInteger)index NS_REQUIRES_SUPER;

...
@interface HFCategoryBaseView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) HFCategoryCollectionView *collectionView;
...
@implementation HFCategoryBaseView
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    HFCategoryBaseCellModel *cellModel = self.dataSource[index];
    cellModel.selectedType = HFCategoryCellSelectedTypeUnknown;
    [self refreshCellModel:cellModel index:index];
    HFCategoryBaseCell *cell = (HFCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    [cell reloadData:cellModel];
}

- (void)selectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:HFCategoryCellSelectedTypeCode];
}

- (void)layoutSubviews {
    [super layoutSubviews];

    //部分使用者为了适配不同的手机屏幕尺寸，HFCategoryView的宽高比要求保持一样，所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。所以，为了规避这个问题，在这里对高度统一向下取整。
    //如果向下取整导致了你的页面异常，请自己重新设置HFCategoryView的高度，保证为整数即可。
...
- (void)setDelegate:(id<HFCategoryViewDelegate>)delegate {
    _delegate = delegate;
...
- (void)setListContainer:(id<HFCategoryViewListContainer>)listContainer {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    HFCategoryBaseCellModel *cellModel = self.dataSource[indexPath.item];
    cellModel.selectedType = HFCategoryCellSelectedTypeUnknown;
    [(HFCategoryBaseCell *)cell reloadData:cellModel];
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    BOOL isTransitionAnimating = NO;
    for (HFCategoryBaseCellModel *cellModel in self.dataSource) {
...
- (CGFloat)getContentEdgeInsetLeft {
    if (self.contentEdgeInsetLeft == HFCategoryViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (CGFloat)getContentEdgeInsetRight {
    if (self.contentEdgeInsetRight == HFCategoryViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (void)clickSelectItemAtIndex:(NSInteger)index {
    ...
    [self selectCellAtIndex:index selectedType:HFCategoryCellSelectedTypeClick];
}

- (void)scrollSelectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:HFCategoryCellSelectedTypeScroll];
}
...
@implementation HFCategoryBaseView (UISubclassingBaseHooks)

- (CGRect)getTargetCellFrame:(NSInteger)targetIndex {
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i ++) {
        HFCategoryBaseCellModel *cellModel = self.dataSource[i];
        CGFloat cellWidth;
    ...
    CGFloat width;
    HFCategoryBaseCellModel *selectedCellModel = self.dataSource[targetIndex];
...
- (CGRect)getTargetSelectedCellFrame:(NSInteger)targetIndex selectedType:(HFCategoryCellSelectedType)selectedType {
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i ++) {
        HFCategoryBaseCellModel *cellModel = self.dataSource[i];
...
- (void)initializeData {
    ...
    _selectedIndex = 0;
    _cellWidth = HFCategoryViewAutomaticDimension;
    ...
    _contentEdgeInsetLeft = HFCategoryViewAutomaticDimension;
    _contentEdgeInsetRight = HFCategoryViewAutomaticDimension;
...
- (void)initializeViews {
    ...
    _collectionView = [[HFCategoryCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)refreshState {
    ...
    if (self.isAverageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(HFCategoryBaseCellModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(HFCategoryBaseCellModel * cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (CGRectEqualToRect(self.contentScrollView.frame, CGRectZero) && self.contentScrollView.superview != nil) {
        //某些情况系统会出现HFCategoryView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现contentScrollView的frame为zero时，强行触发其父视图链里面已经有frame的一个父视图的layoutSubviews方法。
        //比如HFSegmentedListContainerView会将contentScrollView包裹起来使用，该情况需要HFSegmentedListContainerView.superView触发布局更新
...
- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(HFCategoryCellSelectedType)selectedType {
    ...
    if (self.selectedIndex == targetIndex) {
        ...
        if (selectedType == HFCategoryCellSelectedTypeCode) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
        }else if (selectedType == HFCategoryCellSelectedTypeClick) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
        ...
        }else if (selectedType == HFCategoryCellSelectedTypeScroll) {
    ...
    HFCategoryBaseCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    ...
    //刷新当前选中的和将要选中的cell
    HFCategoryBaseCell *lastCell = (HFCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    HFCategoryBaseCell *selectedCell = (HFCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
    [selectedCell reloadData:selectedCellModel];

    if (self.scrollingTargetIndex != -1 && self.scrollingTargetIndex != targetIndex) {
        HFCategoryBaseCellModel *scrollingTargetCellModel = self.dataSource[self.scrollingTargetIndex];
        ...
        HFCategoryBaseCell *scrollingTargetCell = (HFCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.scrollingTargetIndex inSection:0]];
    ...
    if (selectedType == HFCategoryCellSelectedTypeClick ||
        selectedType == HFCategoryCellSelectedTypeCode) {
...
- (void)refreshSelectedCellModel:(HFCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(HFCategoryBaseCellModel *)unselectedCellModel {
    ...
    if (self.isCellWidthZoomEnabled) {
        if (selectedCellModel.selectedType == HFCategoryCellSelectedTypeCode ||
            selectedCellModel.selectedType == HFCategoryCellSelectedTypeClick) {
            ...
            self.animator.progressCallback = ^(CGFloat percent) {
                ...
                selectedCellModel.cellWidthCurrentZoomScale = [HFCategoryFactory interpolationFrom:selectedCellModel.cellWidthNormalZoomScale to:selectedCellModel.cellWidthSelectedZoomScale percent:percent];
                ...
                unselectedCellModel.cellWidthCurrentZoomScale = [HFCategoryFactory interpolationFrom:unselectedCellModel.cellWidthSelectedZoomScale to:unselectedCellModel.cellWidthNormalZoomScale percent:percent];
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (self.animator.isExecuting) {
            ...
            for (HFCategoryBaseCellModel *model in self.dataSource) {
                if (model.isSelected) {
        ...
        if (self.isCellWidthZoomEnabled && self.isCellWidthZoomScrollGradientEnabled) {
            HFCategoryBaseCellModel *leftCellModel = (HFCategoryBaseCellModel *)self.dataSource[baseIndex];
            HFCategoryBaseCellModel *rightCellModel = (HFCategoryBaseCellModel *)self.dataSource[baseIndex + 1];
            leftCellModel.cellWidthCurrentZoomScale = [HFCategoryFactory interpolationFrom:leftCellModel.cellWidthSelectedZoomScale to:leftCellModel.cellWidthNormalZoomScale percent:remainderRatio];
            leftCellModel.cellWidth = [self getCellWidthAtIndex:leftCellModel.index] * leftCellModel.cellWidthCurrentZoomScale;
            rightCellModel.cellWidthCurrentZoomScale = [HFCategoryFactory interpolationFrom:rightCellModel.cellWidthNormalZoomScale to:rightCellModel.cellWidthSelectedZoomScale percent:remainderRatio];
...
- (Class)preferredCellClass {
    return HFCategoryBaseCell.class;
}

...
@implementation HFCategoryTitleView
...
- (void)initializeData {
    ...
    _titleLabelAnchorPointStyle = HFCategoryTitleLabelAnchorPointStyleCenter;
}
...
- (Class)preferredCellClass {
    return [HFCategoryTitleCell class];
}

- (void)refreshDataSource {
    NSMutableArray *tempArray = [NSMutableArray arrayWithCapacity:self.titles.count];
    for (int i = 0; i < self.titles.count; i++) {
        HFCategoryTitleCellModel *cellModel = [[HFCategoryTitleCellModel alloc] init];
...
- (void)refreshSelectedCellModel:(HFCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(HFCategoryBaseCellModel *)unselectedCellModel {
    ...
    HFCategoryTitleCellModel *myUnselectedCellModel = (HFCategoryTitleCellModel *)unselectedCellModel;
    HFCategoryTitleCellModel *myselectedCellModel = (HFCategoryTitleCellModel *)selectedCellModel;
    if (self.isSelectedAnimationEnabled && (selectedCellModel.selectedType == HFCategoryCellSelectedTypeClick || selectedCellModel.selectedType == HFCategoryCellSelectedTypeCode)) {
...
- (void)refreshLeftCellModel:(HFCategoryBaseCellModel *)leftCellModel rightCellModel:(HFCategoryBaseCellModel *)rightCellModel ratio:(CGFloat)ratio {
    ...
    HFCategoryTitleCellModel *leftModel = (HFCategoryTitleCellModel *)leftCellModel;
    HFCategoryTitleCellModel *rightModel = (HFCategoryTitleCellModel *)rightCellModel;

    if (self.isTitleLabelZoomEnabled && self.isTitleLabelZoomScrollGradientEnabled) {
        leftModel.titleLabelCurrentZoomScale = [HFCategoryFactory interpolationFrom:self.titleLabelZoomScale to:1.0 percent:ratio];
        rightModel.titleLabelCurrentZoomScale = [HFCategoryFactory interpolationFrom:1.0 to:self.titleLabelZoomScale percent:ratio];
    }

    if (self.isTitleLabelStrokeWidthEnabled) {
        leftModel.titleLabelCurrentStrokeWidth = [HFCategoryFactory interpolationFrom:leftModel.titleLabelSelectedStrokeWidth to:leftModel.titleLabelNormalStrokeWidth percent:ratio];
        rightModel.titleLabelCurrentStrokeWidth = [HFCategoryFactory interpolationFrom:rightModel.titleLabelNormalStrokeWidth to:rightModel.titleLabelSelectedStrokeWidth percent:ratio];
    }

    if (self.isTitleColorGradientEnabled) {
        leftModel.titleCurrentColor = [HFCategoryFactory interpolationColorFrom:self.titleSelectedColor to:self.titleColor percent:ratio];
        rightModel.titleCurrentColor = [HFCategoryFactory interpolationColorFrom:self.titleColor to:self.titleSelectedColor percent:ratio];
    }
}
...
- (void)refreshCellModel:(HFCategoryBaseCellModel *)cellModel index:(NSInteger)index {
    ...
    HFCategoryTitleCellModel *model = (HFCategoryTitleCellModel *)cellModel;

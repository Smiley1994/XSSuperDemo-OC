...
@implementation HFCategoryTitleCell
...
- (void)layoutSubviews {
    ...
    HFCategoryTitleCellModel *myCellModel = (HFCategoryTitleCellModel *)self.cellModel;
    switch (myCellModel.titleLabelAnchorPointStyle) {
        case HFCategoryTitleLabelAnchorPointStyleCenter: {
        ...
        case HFCategoryTitleLabelAnchorPointStyleTop: {
        ...
        case HFCategoryTitleLabelAnchorPointStyleBottom: {
...
- (void)reloadData:(HFCategoryBaseCellModel *)cellModel {
    ...
    HFCategoryTitleCellModel *myCellModel = (HFCategoryTitleCellModel *)cellModel;
    ...
    if (myCellModel.isTitleLabelStrokeWidthEnabled) {
        if (myCellModel.isSelectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            HFCategoryCellSelectedAnimationBlock block = [self preferredTitleStrokeWidthAnimationBlock:myCellModel attributedString:attributedString];
            [self addSelectedAnimationBlock:block];
        } else {
    ...
    if (myCellModel.isTitleLabelMaskEnabled) {
        ...
        self.titleLabel.layer.mask = nil;
        if (myCellModel.isSelectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            HFCategoryCellSelectedAnimationBlock block = [self preferredTitleColorAnimationBlock:myCellModel];
            [self addSelectedAnimationBlock:block];
        } else {
...
- (HFCategoryCellSelectedAnimationBlock)preferredTitleZoomAnimationBlock:(HFCategoryTitleCellModel *)cellModel baseScale:(CGFloat)baseScale {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            //将要选中，scale从小到大插值渐变
            cellModel.titleLabelCurrentZoomScale = [HFCategoryFactory interpolationFrom:cellModel.titleLabelNormalZoomScale to:cellModel.titleLabelSelectedZoomScale percent:percent];
        } else {
            //将要取消选中，scale从大到小插值渐变
            cellModel.titleLabelCurrentZoomScale = [HFCategoryFactory interpolationFrom:cellModel.titleLabelSelectedZoomScale to:cellModel.titleLabelNormalZoomScale percent:percent];
        }
...
- (HFCategoryCellSelectedAnimationBlock)preferredTitleStrokeWidthAnimationBlock:(HFCategoryTitleCellModel *)cellModel attributedString:(NSMutableAttributedString *)attributedString {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleLabelCurrentStrokeWidth = [HFCategoryFactory interpolationFrom:cellModel.titleLabelNormalStrokeWidth to:cellModel.titleLabelSelectedStrokeWidth percent:percent];
        } else {
            //将要取消选中，StrokeWidth从大到小插值渐变
            cellModel.titleLabelCurrentStrokeWidth = [HFCategoryFactory interpolationFrom:cellModel.titleLabelSelectedStrokeWidth to:cellModel.titleLabelNormalStrokeWidth percent:percent];
        }
...
- (HFCategoryCellSelectedAnimationBlock)preferredTitleColorAnimationBlock:(HFCategoryTitleCellModel *)cellModel {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleCurrentColor = [HFCategoryFactory interpolationColorFrom:cellModel.titleNormalColor to:cellModel.titleSelectedColor percent:percent];
        } else {
            //将要取消选中，textColor从titleSelectedColor到titleNormalColor插值渐变
            cellModel.titleCurrentColor = [HFCategoryFactory interpolationColorFrom:cellModel.titleSelectedColor to:cellModel.titleNormalColor percent:percent];
        }
